...
dist_min=10**(-1.2)
dist_max=10**0.7#Specific as Trowland+12 is.
...
for dist_bin in range(tot_dist_bins):
    ...
    results[dist_bin,5]=low_int+(hi_int-low_int)/2 #bin value, at the end to avoid changing other
...
plt.savefig('/scratch/GAMNSCM2/halo_halo_plts/%s/%s/halohalodpincrem%s_%s_%s_%s.png'%(sim_type,cosmology,sim_type,cosmology,snapshot,particles_filt))
...
posterior_plt(cosmology,diction_2,results,hist_bins,sim_sz,grid_nodes,smooth_scl,tot_dist_bins,particles_filt,lss_type,method,sim_type,snapshot,den_type,dp_mthd,no_dist=no_dist)
