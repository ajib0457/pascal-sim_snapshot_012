...
#partcl_100=np.where(npart>=75)#filter out halos with <100 particles
...
inputfile=open("/project/GAMNSCM2/snapshot_012_LSS_class/correl/DTFE/files/output_files/dotproduct/spin_lss/DTFE_grid%d_spin_store_%dbins_fil_Log%s-%s_smth%skpc_binr.pkl"%(grid_nodes,n_dot_prod,round(low_int_mass,2),round(hi_int_mass,2),std_dev_phys),'rb')
...
output=open("/scratch/GAMNSCM2/snapshot_012_LSS_class/correl/DTFE/files/output_files/mcmc/spin_lss/flt_chain_%sburnin_%ssteps_%snwalkrs_grid%d_fil_Log%s-%s_smth%skpc_spin_lss.pkl"%(burn_in,steps_wlk,nwalkers,grid_nodes,round(low_int_mass,2),round(hi_int_mass,2),round(std_dev_phys/1000,3)), 'wb')
