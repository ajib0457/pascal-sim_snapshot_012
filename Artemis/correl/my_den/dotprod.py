...
particles_filt=50  #Halos to filter out based on number of particles, ONLY for Dot Product Spin-LSS(SECTION 5.)
Mass_res=8.16823864*10**10 #snapshot f['mass'] 
...
for part in range(total_lss_parts):
    ...
    f=h5py.File("/scratch/GAMNSCM2/snapshot_012_LSS_class/correl/my_den/files/output_files/eigvecs/fil_sim%s_recon_vecs_DTFE_gd%d_smth%sMpc_%d.h5" %(sim_sz,grid_nodes,smooth_scl,part), 'r')
    ...
    f.close()
    f2=h5py.File("/scratch/GAMNSCM2/snapshot_012_LSS_class/correl/my_den/files/output_files/eigvecs/fil_sim%s_recon_vecs_DTFE_gd%d_smth%sMpc_%d_mask.h5" %(sim_sz,grid_nodes,smooth_scl,part), 'r')
...
f=h5py.File("/scratch/GAMNSCM2/snapshot_012_LSS_class/catalogs/pascal_VELOCIraptor_allhalos_xyz_vxyz_jxyz_m_r.h5", 'r')
...
filehandler = open('/scratch/GAMNSCM2/snapshot_012_LSS_class/correl/my_den/files/output_files/dotproduct/spin_lss/myden_dp_LSS%s_spin_sim%sMpc_grid%s_smth%sMpc_%sbins_partclfilt%s.pkl'%(lss_type,sim_sz,grid_nodes,smooth_scl,tot_mass_bins,particles_filt),"wb")
